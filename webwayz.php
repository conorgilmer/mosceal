<?php 
$thisPage = "Webwayz";
include ("inc/header.php");
include ("inc/menu.php");
?>

<div id="fb-root"></div>
<script>(function(d, s, id) {
  var js, fjs = d.getElementsByTagName(s)[0];
  if (d.getElementById(id)) return;
  js = d.createElement(s); js.id = id;
  js.src = "//connect.facebook.net/en_US/sdk.js#xfbml=1&version=v2.5&appId=607946642599752";
  fjs.parentNode.insertBefore(js, fjs);
}(document, 'script', 'facebook-jssdk'));</script>

		   <!-- main page content -->
		  <div class="wrapper">
		  <!-- above the containers to push down the footer -->

		      <div class="container">

			<div class="row">

			<div class="span3">
<a href="https://twitter.com/webwayz" class="twitter-follow-button" data-show-count="false">Follow @webwayz</a> <script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0],p=/^http:/.test(d.location)?'http':'https';if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src=p+'://platform.twitter.com/widgets.js';fjs.parentNode.insertBefore(js,fjs);}}(document, 'script', 'twitter-wjs');</script>

<br/>

<div class="fb-follow" data-href="https://www.facebook.com/webwayz" data-layout="standard" data-show-faces="true"></div>

			</div>

			<div class="span3">
<a href="https://twitter.com/intent/tweet?screen_name=webwayz" class="twitter-mention-button" data-related="webwayz">Tweet to @webwayz</a> <script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0],p=/^http:/.test(d.location)?'http':'https';if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src=p+'://platform.twitter.com/widgets.js';fjs.parentNode.insertBefore(js,fjs);}}(document, 'script', 'twitter-wjs');</script>
<br/>
<div class="fb-share-button" data-href="http://www.facebook.com/webwayz" data-layout="button_count"></div>
			</div>

			<div class="span3">

<a href="https://twitter.com/intent/tweet?screen_name=webwayz" class="twitter-mention-button" data-related="webwayz">Tweet to @webwayz</a> <script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0],p=/^http:/.test(d.location)?'http':'https';if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src=p+'://platform.twitter.com/widgets.js';fjs.parentNode.insertBefore(js,fjs);}}(document, 'script', 'twitter-wjs');</script>
<br/>
<div class="fb-send" data-href="http://www.facebook.com/webwayz"></div>
			</div>

			<div class="span3">
<a href="https://twitter.com/share" class="twitter-share-button"{count} data-url="http://www.webwayz.com" data-via="webwayz">Tweet</a> <script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0],p=/^http:/.test(d.location)?'http':'https';if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src=p+'://platform.twitter.com/widgets.js';fjs.parentNode.insertBefore(js,fjs);}}(document, 'script', 'twitter-wjs');</script>

<br/>
<div class="fb-like" data-href="http://www.facebook.com/webwayz" data-layout="standard" data-action="like" data-show-faces="true" data-share="false"></div>

			</div>

			</div>
			<div class="row">

	<!-- Page stuff -->

	<h1>Webwayz Social Media</h1>


<h3>Twitter and Facebook</h3>
			<div class="span6">
<a class="twitter-timeline" href="https://twitter.com/webwayz" data-widget-id="680086568187654144">Tweets by @webwayz</a> <script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0],p=/^http:/.test(d.location)?'http':'https';if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src=p+"://platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script>			</div>
			<div class="span6">
			</div>
			</div>

         </div> 
<?php include ("inc/footer.php") ?>



